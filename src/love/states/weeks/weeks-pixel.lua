...
return {
	...
	updateUI = function(self, dt, canvas)
		...
		for i = 1, 4 do
					...
					for i = 1, #boyfriendNote do
						if boyfriendNote[i] and boyfriendNote[i]:getAnimName() == "on" then
							if (not settings.downscroll and boyfriendNote[i].y - musicPos <= -31) or (settings.downscroll and boyfriendNote[i].y - musicPos >= 31) then
								...
								if notePos <= 4.9 then -- "Sick"
									score = score + 350
									ratingAnim = "sick"
								elseif notePos <= 12.9 then -- "Good"
									score = score + 200
									ratingAnim = "good"
								elseif notePos <= 16.9 then -- "Bad"
									score = score + 100
									ratingAnim = "bad"
								else -- "Shit"
									if settings.kadeInput then
				...
				if #enemyNote > 0 then
					for i = 1, #enemyNote do
						if enemyNote[i] and enemyNote[i]:getAnimName() == "on" then
							if (not settings.downscroll and enemyNote[i].y - musicPos <= -31) or (settings.downscroll and enemyNote[i].y - musicPos >= 31) then
								local notePos
								local ratingAnim
								...
								if settings.downscroll then
									notePos = math.abs(55 - (enemyNote[i].y - musicPos))
								else
									notePos = math.abs(-55 - (enemyNote[i].y - musicPos))
								end
			...
			if #enemyNote > 0 then
				if input:down(curInput2) then
					if ((not settings.downscroll and enemyNote[1].y - musicPos <= -55) or (settings.downscroll and enemyNote[1].y - musicPos >= 55)) and (enemyNote[1]:getAnimName() == "hold" or enemyNote[1]:getAnimName() == "end") then
						voices:setVolume(1)
