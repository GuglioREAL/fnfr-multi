...
return {
	...
	updateUI = function(self, dt)
		...
		for i = 1, 4 do
			...
			if #enemyNote > 0 then
				if (not settings.downscroll and enemyNote[1].y - musicPos < -500) or (settings.downscroll and enemyNote[1].y - musicPos > 500) then
			...
			if input:pressed(curInput2) then
				local success = false
						...
						if enemyNote[i] and enemyNote[i]:getAnimName() == "on" then
							if (not settings.downscroll and enemyNote[i].y - musicPos <= -280) or (settings.downscroll and enemyNote[i].y - musicPos >= 280) then
								...
								if not settings.kadeInput or success then
									...
									if useAltAnims then
										self:safeAnimate(enemy, curAnim .. " alt", false, 3)
									else
			...
			if #boyfriendNote > 0 and input:down(curInput) and ((not settings.downscroll and boyfriendNote[1].y - musicPos <= -400) or (settings.downscroll and boyfriendNote[1].y - musicPos >= 400)) and (boyfriendNote[1]:getAnimName() == "hold" or boyfriendNote[1]:getAnimName() == "end") then
				voices:setVolume(1)
			...
			if #enemyNote > 0 and input:down(curInput2) and ((not settings.downscroll and enemyNote[1].y - musicPos <= -400) or (settings.downscroll and enemyNote[1].y - musicPos >= 400)) and (enemyNote[1]:getAnimName() == "hold" or enemyNote[1]:getAnimName() == "end") then
				voices:setVolume(1)
				...
				if (not enemy:isAnimated()) or boyfriend:getAnimName() == "idle" then 
					if useAltAnims then
						self:safeAnimate(enemy, curAnim .. " alt", true, 3)
					else
	...
	drawUI = function(self)
		love.graphics.push()
			...
			if settings.downscroll then
				...
				love.graphics.print("Enemy Score: " .. score2 .. "           Boyfriend Score: " .. score, -375, 400)
			end
